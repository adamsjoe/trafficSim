...
export default function TrafficMap() {
  ...
  const [status, setStatus] = useState<StatusState>({
    cls: "",
    msg: "Enter a location and click Load Roads",
  });
  ...
  useEffect(() => {
    closureModeRef.current = closureMode;
  }, [closureMode]);
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      trailCanvas.style.cssText =
        "position:absolute;top:0;left:0;pointer-events:none;z-index:600;";
      ...
      try {
        ...
        console.warn(
          "WebGL unavailable, falling back to 2D canvas rendering",
          e,
        );
      }
      ...
      map.on("movestart zoomstart", () => {
        trailCtxRef.current?.clearRect(
          0,
          0,
          trailCanvasRef.current!.width,
          trailCanvasRef.current!.height,
        );
  ...
  // ── Road closure ─────────────────────────────────────────────────────────────
  const toggleRoadClosure = useCallback(
    (pd: PolylineData, graph: RoadGraph) => {
      pd.closed = !pd.closed;
      for (let i = 0; i < pd.nodeSeq.length - 1; i++) {
        const a = pd.nodeSeq[i],
          b = pd.nodeSeq[i + 1];
        if (pd.closed) {
          graph.closedEdges.add(`${a}-${b}`);
          graph.closedEdges.add(`${b}-${a}`);
        } else {
          graph.closedEdges.delete(`${a}-${b}`);
          graph.closedEdges.delete(`${b}-${a}`);
        }
      }
      if (pd.closed) {
        pd.polyline.setStyle({
          color: "#ff2244",
          weight: pd.originalWeight + 1,
          opacity: 0.9,
          dashArray: "8 5",
        });
      } else {
        pd.polyline.setStyle({
          color: pd.originalColor,
          weight: pd.originalWeight,
          opacity: paramsRef.current.roadOpacity,
          dashArray: "",
        });
      }
  ...
  const handleFetchRoads = useCallback(async () => {
    ...
    try {
      ...
      for (const el of data.elements) {
        ...
        const pts = nodeSeq.map(
          (id) =>
            [graph.nodes[id].lat, graph.nodes[id].lng] as [number, number],
        );
        ...
        const pd: PolylineData = {
          polyline: visual,
          nodeSeq,
          originalColor: cfg.color,
          originalWeight: cfg.weight,
          closed: false,
        };
        ...
        hit.on("mouseover", () => {
          if (closureModeRef.current && !pd.closed)
            visual.setStyle({
              color: "#ffcc00",
              opacity: 1,
              weight: pd.originalWeight + 1,
        ...
        hit.on("click", (e) => {
          ...
          if (closureModeRef.current) toggleRoadClosure(pd, graph);
        });
      ...
      if (nc < 3) {
        setStatus({
          cls: "error",
          msg: "Too few nodes — zoom out or enable more road types",
        });
        return;
      }
      ...
      setStatus({
        cls: "active",
        msg: `${fmtN(nc)} nodes · ${fmtN(ec)} edges ready`,
      });
    } catch (err) {
      setStatus({
        cls: "error",
  ...
  const handleReopenAll = useCallback(() => {
    ...
    [...polylineDataRef.current].forEach((pd) => {
      if (pd.closed) toggleRoadClosure(pd, graph);
    });
  }, [toggleRoadClosure]);
  ...
  const syncCarCount = useCallback((graph: RoadGraph) => {
    ...
    while (carsRef.current.length < target)
      carsRef.current.push(new Car(graph));
  ...
  const startSim = useCallback(() => {
    ...
    const tick = (now: number) => {
      ...
      fpsBufRef.current =
        fpsBufRef.current * 0.88 + (1000 / Math.max(dt, 1)) * 0.12;
        ...
        if (doTrails && trailCtx) {
          ...
          if (car.trail.length > 1) {
            ...
            for (let i = 1; i < car.trail.length; i++)
              trailCtx.lineTo(car.trail[i].x, car.trail[i].y);
    ...
    <div className="layout">
      ...
      <header className="topbar">
        <span className="brand">
          Traffic<span className="brand-dim">Sim</span>
        </span>
        ...
        <div className="search-wrap">
          ...
          />
          <button className="btn btn-locate" onClick={handleSearch}>
            Locate
          </button>
          <button className="btn btn-fetch" onClick={handleFetchRoads}>
            Load Roads
          </button>
        </div>
      ...
      <div className="main">
        <div className="map-wrap">
          ...
          {params.carCount > TRAIL_MAX_CARS &&
            params.trailLength > 0 &&
            simStatus === "running" && (
              <div
                className="closure-hint"
                style={{
                  bottom: closureMode ? 70 : 30,
                  borderColor: "#3d5068",
                  color: "#5a7a9a",
                }}
              >
